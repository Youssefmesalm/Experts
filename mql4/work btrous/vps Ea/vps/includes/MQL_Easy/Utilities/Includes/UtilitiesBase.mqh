...
#include <MQL_Easy/Error/Error.mqh>
...
//|    return the Spread
//+------------------------------------------------------------------+
long CUtilitiesBase::Spread(void)
{
   ResetLastError();
   long SpreadTemp = SymbolInfoInteger(this.Symbol, SYMBOL_SPREAD);
   //-- check if any error occurs
   if (this.Error.CheckLastError(true, __FUNCTION__))
      return WRONG_VALUE;
   //--
...
double CUtilitiesBase::NormalizePrice(double pricePar, ENUM_ROUNDING roundPar = 0)
{
   ...
   {
   case ROUNDING_UP:
      return (MathCeil(pricePar / tickSize) * tickSize);
   case ROUNDING_DOWN:
      return (MathFloor(pricePar / tickSize) * tickSize);
   default:
      return (MathRound(pricePar / tickSize) * tickSize);
   }
}
...
double CUtilitiesBase::NormalizeVolume(double lotsPar, ENUM_ROUNDING roundPar = 0)
{
   ...
   {
   case ROUNDING_UP:
      return (MathCeil(lotsPar / lotStep) * lotStep);
   case ROUNDING_DOWN:
      return (MathFloor(lotsPar / lotStep) * lotStep);
   default:
      return (MathRound(lotsPar / lotStep) * lotStep);
   }
}
...
bool CUtilitiesBase::SltpConvert(ENUM_SLTP_TYPE sltpPar, int typePar, double openPricePar, double slPar, double tpPar, double &slRef, double &tpRef)
{
   ...
   if (pipTemp == WRONG_VALUE || pointTemp == WRONG_VALUE)
      return false;
   //--
   if (typePar == 0 || typePar == 2 || typePar == 4)
   {
      ...
      case SLTP_PRICE:
      {
         if (slPar == WRONG_VALUE)
            slRef = WRONG_VALUE;
         else if (slPar == 0)
            slRef = 0;
         else
            slRef = slPar;
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
      ...
      case SLTP_PIPS:
      {
         if (slPar == WRONG_VALUE)
            ...
            slRef = MathAbs(openPricePar - slPar * pipTemp);
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
            tpRef = openPricePar + tpPar * pipTemp;
         break;
      }
      case SLTP_POINTS:
      {
         if (slPar == WRONG_VALUE)
            ...
            slRef = MathAbs(openPricePar - slPar * pointTemp);
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
            tpRef = openPricePar + tpPar * pointTemp;
         break;
      }
      case SLTP_PERCENTAGE:
      {
         if (slPar == WRONG_VALUE)
            ...
            slRef = MathAbs(openPricePar - this.NormalizePrice(openPricePar * (slPar / 100)));
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
            tpRef = openPricePar + this.NormalizePrice(openPricePar * (tpPar / 100));
         break;
      }
      default:
         result = this.Error.CreateErrorCustom("Wrong input on parameter ENUM_SLTP_TYPE = " + EnumToString(sltpPar), false, false, __FUNCTION__);
      }
   }
   else if (typePar == 1 || typePar == 3 || typePar == 5)
   {
      ...
      case SLTP_PIPS:
      {
         if (slPar == WRONG_VALUE)
            ...
            slRef = openPricePar + slPar * pipTemp;
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
            tpRef = MathAbs(openPricePar - tpPar * pipTemp);
         break;
      }
      case SLTP_POINTS:
      {
         if (slPar == WRONG_VALUE)
            ...
            slRef = openPricePar + slPar * pointTemp;
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
            tpRef = MathAbs(openPricePar - tpPar * pointTemp);
         break;
      }
      case SLTP_PERCENTAGE:
      {
         if (slPar == WRONG_VALUE)
            ...
            slRef = openPricePar + this.NormalizePrice(openPricePar * (slPar / 100));
         if (tpPar == WRONG_VALUE)
            tpRef = WRONG_VALUE;
         else if (tpPar == 0)
            tpRef = 0;
         else
            tpRef = MathAbs(openPricePar - this.NormalizePrice(openPricePar * (tpPar / 100)));
         break;
      }
      default:
         result = this.Error.CreateErrorCustom("Wrong input on value ENUM_SLTP_TYPE = " + EnumToString(sltpPar), false, false, __FUNCTION__);
      }
   }
   else
   {
