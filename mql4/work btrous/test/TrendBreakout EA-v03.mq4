...
//---
   ...
   double Value0_1 = iCustom(Symbol(),Time1,"Renko Super-signals_v3 double",PeriodWeekChannel,PeriodMainChannel,alertsOn,alertsMessage,alertsSound,alertsEmail,soundFile,0,1);
   double Value1_1 = iCustom(Symbol(),Time1,"Renko Super-signals_v3 double",PeriodWeekChannel,PeriodMainChannel,alertsOn,alertsMessage,alertsSound,alertsEmail,soundFile,1,1);
   ...
   if(Value1_1!=0 && Value1_1!=EMPTY_VALUE)
     {
           CloseOrderBuy();

     }
   if(Value0_1!=0 && Value0_1!=EMPTY_VALUE)
     {
            ...
            if(Lasttradetime!=Time[0])
              {
               ...
               if(TakeProfit3==0)
                 ...
                 {
                  tp3= Ask+TakeProfit3*Point;
                 }

               if(OrderSend(Symbol(),OP_BUY,Lots1,Ask,30,Value1,tp1,"Buy1",MagicNumber,0,clrBlue)==false)
                 {
                  GetLastError();
                 }
               if(OrderSend(Symbol(),OP_BUY,Lots2,Ask,30,Value1,tp2,"Buy2",MagicNumber,0,clrBlue)==false)
                 {
                  GetLastError();
                 }
               if(OrderSend(Symbol(),OP_BUY,Lots3,Ask,30,Value1,tp3,"Buy3",MagicNumber,0,clrBlue)==false)
                 {
                  GetLastError();
                 }
            ...
            if(Lasttradetime!=Time[0])
              {
               ...
               if(TakeProfit3==0)
                 ...
                 {
                  tp3= Bid-TakeProfit3*Point;
                 }

               if(OrderSend(Symbol(),OP_SELL,Lots1,Bid,30,Value0,tp1,"Sell1",MagicNumber,0,clrRed)==false)
                 {
                  GetLastError();
                 }
               if(OrderSend(Symbol(),OP_SELL,Lots2,Bid,30,Value0,tp2,"Sell2",MagicNumber,0,clrRed)==false)
                 {
                  GetLastError();
                 }
               if(OrderSend(Symbol(),OP_SELL,Lots3,Bid,30,Value0,tp3,"Sell3",MagicNumber,0,clrRed)==false)
                 {
                  GetLastError();
                 }
