...
int startBS(int MagicNoBS)
{
   ...
   //Make Order         
   if(true) {
      if(direction ==OP_BUY && MagicNoBS ==MagicKey[1]&& CurTime()-lastordertime > sometimelimit){
         MakeBuyOrder(InitLots);
         lastordertime= CurTime();
         }
      if(direction ==OP_SELL && MagicNoBS ==MagicKey[2]&& CurTime()-lastordertime > sometimelimit){
