...
input double Lot                    =1;
input EntryType entry_Type          =Points;
input double Entry= 540;
...
input bool Use_Activation_Time          =false;
input timeType Activation_Time_Type =GMT;
...
input timeType Close_Time_type = GMT;
input bool close_of_active_trades  = true;
input string time_to_breakEven      ="00:00";
input double BreakEven_point        =0 ;
input string Time_to_close_Active_Trades = "08:00";
input bool close_of_inactive_trades = true;
input Methods Inactive_Trades_Methods=Delete;
input string TimeTo_Delete_and_Suspend_Inactive_Trades ="08:00";
input string TimeTo_stop_suspension_Inactive_Trades ="09:00";
//variables
...
int DTD[18]= {480,300,460,292,240,540,340,560,320,480,320,520,520,290,460,280,360,296};
int DAD[18]= {480,300,360,292,240,540,340,560,320,480,320,520,520,290,460,280,360,296};
   ...
   if(Pos.GroupTotal()==0&&closewithtp==true)
     {
            ...
            if(entry_Type==Points)
              {
               if(Trade_Type==Buy||Trade_Type==Buystop||Trade_Type==Selllimit)
                 {
                  entry=tool.NormalizePrice(tool.Bid()+Entry*tool.Point());
                  entryPut=true;
                 }
      ...
      if((close_of_inactive_trades&&!TimeFilter(TimeTo_Delete_and_Suspend_Inactive_Trades,TimeTo_stop_suspension_Inactive_Trades))||!close_of_inactive_trades)
        {
...
void Closing()
  {
   ...
   if(close_of_inactive_trades&&ClosingTimeFilter(TimeTo_Delete_and_Suspend_Inactive_Trades)&&!suspendHappen)
     {
      if(order.GroupTotal()>0)
        {
   ...
   if(close_of_active_trades&&ClosingTimeFilter(time_to_breakEven))
     {
         ...
         if(type==ORDER_TYPE_BUY)
           {
            ...
            if(p>tool.Ask())
              {
               Pos[i].Modify(Pos[i].GetPriceOpen(),Pos[i].GetTakeProfit(),SLTP_PRICE);
               BE_happen=true;
               BE_reset=false;
              }
           }
...
void ReSet()
  {
   ...
   if(which==reset&&!BE_happen&&size==0&&MathAbs(fristtp-tool.Bid())/tool.Point()>=DA&&enable_Post_Reset)
     {
      order.GroupCloseAll(30);
      closewithtp=true;
      closewithBE=false;
