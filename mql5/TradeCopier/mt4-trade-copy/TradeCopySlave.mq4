...
void compare_positions()
{
  ...
  for (int i = 0; i < Size; i++)
  { // for all master orders
    bool found = false;
    for (int j = 0; j < RealSize; j++)
    { // find the right real order
    ...
    if (!found)
    {
      ...
      if (OrdTyp[i] < 2)
      {
            ...
            (OrdTyp[i] == OP_SELL && Price > OrdPrice[i] - PipsTolerance * mp * Point))
        {
          ...
          Print("Price out of tolerance ", DoubleToStr(OrdId[i], 0), ": ", OrdPrice[i], "/", Price);
      }
      else
      {
        // ------ waiting order:
        if (CopyDelayedTrades)
  ...
  for (j = 0; j < RealSize; j++)
  {
    ...
    if (x[j] != 1)
    { //no master order, close the ticket
      ...
      if (RealOrdTyp[j] < 2)
      {
        ...
        if (result < 1)
          Print("Close ", RealOrdId[j], " / ", RealOrdLot[j], " / ", Price, " failed: ", GetLastError());
        if (Balance < AccountBalance())
          Balance = AccountBalance();
      }
      else
