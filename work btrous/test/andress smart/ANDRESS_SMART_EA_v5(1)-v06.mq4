...
Open_Sell_Buy=3 
 ...
 enum Closemod {Profit_Loss,Profit_in_Pip};
...
if((Market_Order_Type==Open_Buy || Market_Order_Type==Open_Sell_Buy)&& count_for_type(OP_BUY)==0)
 {
      ...
      if(OrderSend(Symbol(),OP_BUY,Lots,Ask,30,sl,tp,"Buy"+Order_comment ,Magic_number ,0,Green)==false){GetLastError();}
         

 } 
 if((Market_Order_Type==Open_Sell || Market_Order_Type==Open_Sell_Buy)&& count_for_type(OP_SELL)==0)
 {
      if(StopLoss==0){sl=0;}else{sl= Bid+StopLoss*Point*pt;}
      if(TakeProfit==0){tp=0;}else{tp= Bid-TakeProfit*Point*pt;}
      if(OrderSend(Symbol(),OP_SELL,Lots,Bid,30,sl,tp,"Sell"+Order_comment ,Magic_number ,0,Red)==false){GetLastError();}
...
if((Market_Order_Type_5==Open_Buy || Market_Order_Type_5==Open_Sell_Buy))
 ...
 if((count_for_type_1(OP_BUY)>0 || count_for_type_2(OP_BUY)>0 || count_for_type_3(OP_BUY)>0 || count_for_type_4(OP_BUY)>0 || count_for_type_5(OP_BUY)>0) )
 {
   ModifyBuy();
  }
  if((count_for_type_1(OP_SELL)>0 || count_for_type_2(OP_SELL)>0 || count_for_type_3(OP_SELL)>0 || count_for_type_4(OP_SELL)>0 || count_for_type_5(OP_SELL)>0) )
 {
   ModifySell();
  }
 //profit or loss in USD for First Grid  
 if(count_chart_1(Symbol())>0 )
   {
...
//+------------------------------------------------------------------+
//|   Modify SL_TP for First Order  Buy                                                             |
//+------------------------------------------------------------------+
void ModifyBuy()
  {
      for(int i=0; i<OrdersTotal(); i++)
