...
CRiskCalculator ::CRiskCalculator() : m_sl_point(200), m_loss(0), m_profit(0), m_lot(0)
{
   ...
   if (CheckPointer(m_symbol) == POINTER_INVALID)
      Print(__FUNCTION__, " the symbol is invalid");
...
bool CRiskCalculator::CreateChartText(CChartObjectText &label, const string name, const double price, const color Color, const double profit, const double pips)
{
   ...
   string Desc = label.Name() == "market price label" ? "Des: " + DoubleToString(pips, 2) + " spread :" + DoubleToString(m_symbol.Spread(), 2) : x + ": " + DoubleToString(profit, 2) + " Pips: " + DoubleToString(pips, 2) + " Lot: " + DoubleToString(m_lot, 2);
   label.Color(Color);
...
void CRiskCalculator::CalcBuy(void)
{
   ...
   m_sl = NormalizeDouble(m_symbol.Ask() - (m_sl_point * m_symbol.Point()), m_symbol.Digits());
   m_tp = NormalizeDouble(m_symbol.Bid() + ((m_sl_point * ratio) * m_symbol.Point()), m_symbol.Digits());
   m_lot = NormalizeLot(m_account.MaxLotCheck(m_symbol.Name(), ORDER_TYPE_BUY, m_symbol.Bid(), risk));

   if (!CreateLine(m_sl_line, "stop loss", m_sl, clrRed, 2))
      PrintFormat("Can't create Stoploss Line");
   // Calculate loss
   double losspips = m_symbol.Ask() - m_sl_line.Price(0);
   ...
   if (!CreateLine(m_tp_line, "take profit", m_tp, clrGreen, 2))
      PrintFormat("Can't create Stoploss Line");
   // Calculate profit
   double profitpips = m_tp_line.Price(0) - m_symbol.Ask();
...
void CRiskCalculator::CalcSell(void)
{
   ...
   double ratio = StringToDouble(m_ratio_edit.Text()) < 1.00 ? 1 : StringToDouble(m_ratio_edit.Text());
   m_symbol.RefreshRates();
   m_sl = NormalizeDouble(m_symbol.Bid() + (m_sl_point * m_symbol.Point()), m_symbol.Digits());
   m_tp = NormalizeDouble(m_symbol.Ask() - ((m_sl_point * ratio) * m_symbol.Point()), m_symbol.Digits());
   m_lot = NormalizeLot(m_account.MaxLotCheck(m_symbol.Name(), ORDER_TYPE_SELL, m_symbol.Ask(), risk));
   ...
   // Calculate profit
   double profitpips = m_symbol.Ask() - m_tp_line.Price(0);
   ...
   // Calculate loss
   double losspips = m_sl_line.Price(0) - m_symbol.Ask();
...
// Validate Lot volume
double CRiskCalculator::NormalizeLot(const double lot)
{
   double result = 0;
...
void CRiskCalculator::ExecuteClick()
{
   ...
   m_trade.SetDeviationInPoints(90);
   if (m_trade_type == 0)
   {
      m_trade.Sell(m_lot, m_symbol.Name(), m_symbol.Bid(), m_sl, m_tp, "Expert");
      m_trade_type = 0;
      m_tp_line.Delete();
      m_sl_line.Delete();
      m_sl_label.Delete();
   ...
   {
      m_trade.Buy(m_lot, m_symbol.Name(), m_symbol.Ask(), m_sl, m_tp, "Expert");
      m_trade_type = 0;
      m_tp_line.Delete();
      m_sl_line.Delete();
      m_sl_label.Delete();
...
void CRiskCalculator::EnterClick(void)
{
   ...
   if (m_trade_type == 2)
   {
      if (m_market > m_symbol.Ask())
      {

         m_market = m_market_price_line.Price(0);
         if (m_tp > m_market && m_sl < m_market)
         {
            m_trade.BuyStop(m_lot, m_market, m_symbol.Name(), m_sl, m_tp);
            m_trade_type = 0;
            m_tp_line.Delete();
            m_sl_line.Delete();
            m_sl_label.Delete();
         ...
         {
            m_trade.SellLimit(m_lot, m_market, m_symbol.Name(), m_sl, m_tp);
            m_trade_type = 0;
            m_tp_line.Delete();
            m_sl_line.Delete();
            m_sl_label.Delete();
      ...
      else if (m_market < m_symbol.Ask())
      {
         if (m_tp < m_market && m_sl > m_market)
         {
            m_trade.SellStop(m_lot, m_market, m_symbol.Name(), m_sl, m_tp);
            m_trade_type = 0;
            m_tp_line.Delete();
            m_sl_line.Delete();
            m_market_price_line.Delete();
         ...
         {
            m_trade.BuyLimit(m_lot, m_market, m_symbol.Name(), m_sl, m_tp);
            m_trade_type = 0;
            m_tp_line.Delete();
            m_sl_line.Delete();
            m_market_price_line.Delete();
...
void CRiskCalculator::DragTakeProfitLine(void)
{
   ...
   else if (m_trade_type == 2)
   {
      ...
      if (m_market > m_symbol.Ask())
      {
         ...
         double profitpips = m_market_price_line.Price(0) - m_symbol.Ask();
         double pips = profitpips / _Point;
      ...
      {
         double profitpips = m_symbol.Bid() - m_market_price_line.Price(0);
         double pips = profitpips / _Point;
         double Spread = m_symbol.Spread();
         if (m_tp < m_market && m_sl > m_market)
         {
